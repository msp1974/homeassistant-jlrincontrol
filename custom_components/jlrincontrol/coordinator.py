    ...
    async def async_update_data(self, *args):
        ...
        try:
            for vehicle in self.connection.vehicles:
                ...
                jlr_service = JLRService(self.coordinator, vehicle.vin, "update_health_status")
...
class JLRIncontrolUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry) -> None:
        ...
        self.entities = []
        self.pin = config_entry.data.get(CONF_PIN) if config_entry.data.get(CONF_PIN) != "0000" else None
        self.pressure_unit = config_entry.options.get(CONF_PRESSURE_UNIT)
        self.default_climate_temp = config_entry.options.get(CONF_DEFAULT_CLIMATE_TEMP, 21)
        self.default_service_duration = config_entry.options.get(CONF_DEFAULT_SERVICE_DURATION, 24)
        self.update_interval = config_entry.options.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)
        self.health_update_interval = config_entry.options.get(CONF_HEALTH_UPDATE_INTERVAL)
    ...
    async def async_get_user_info(self) -> None:
        ...
        if uoms:
            ...
            user_prefs = UserPreferenceUnits(
                distance=JLR_TO_HASS_UNITS.get(uoms[0], self.hass.config.units.length_unit),
                fuel=JLR_TO_HASS_UNITS.get(uoms[1], self.hass.config.units.volume_unit),
                temp=JLR_TO_HASS_UNITS.get(uoms[2], self.hass.config.units.temperature_unit),
                pressure=self.hass.config.units.pressure_unit
                if self.pressure_unit == "Default"
                else self.pressure_unit,
                energy_regenerated=JLR_TO_HASS_UNITS.get(uoms[4], UnitOfEnergy.KILO_WATT_HOUR),
    ...
    async def async_get_vehicle_attributes(self, vehicle: jlrpy.Vehicle) -> None:
        ...
        if attributes:
            ...
            self.vehicles[vehicle.vin].name = self.vehicles[vehicle.vin].attributes.get("nickname")
            ...
            if "ECC" in self.vehicles[vehicle.vin].supported_services:
                self.vehicles[vehicle.vin].supported_services.extend(["ECCON", "ECCOFF"])
    ...
    async def async_get_vehicle_status(self, vehicle: jlrpy.Vehicle) -> None:
        ...
        if status:
            ...
            if status["vehicleStatus"].get("coreStatus"):
                status_data = {d["key"]: d["value"] for d in status["vehicleStatus"]["coreStatus"]}
            ...
            if status["vehicleStatus"].get("evStatus"):
                status_ev_data = {d["key"]: d["value"] for d in status["vehicleStatus"]["evStatus"]}
                self.vehicles[vehicle.vin].status_ev = dict(sorted(status_ev_data.items()))
        ...
        try:
            climate_temp_data = await self.hass.async_add_executor_job(vehicle.get_rcc_target_value)
    ...
    def get_tracked_statuses(self, vehicle: VehicleData) -> None:
        ...
        # Guardian mode
        vehicle.tracked_status.guardian_mode_active = get_value_match(vehicle.guardian_mode, "status", "ACTIVE")

        # Charging
        vehicle.tracked_status.is_charging = get_value_match(vehicle.status_ev, "EV_IS_CHARGING", "TRUE")

        # Privacy mode status
        vehicle.tracked_status.privacy_mode_enabled = get_value_match(vehicle.status, "PRIVACY_SWITCH", "FALSE")

        # Service mode
        vehicle.tracked_status.service_mode_enabled = get_is_date_active(vehicle.status, "SERVICE_MODE_STOP")

        # Transport mode
        vehicle.tracked_status.transport_mode_enabled = get_is_date_active(vehicle.status, "TRANSPORT_MODE_STOP")
    ...
    async def async_get_guardian_mode_status(self, vehicle: jlrpy.Vehicle) -> None:
        ...
        if "GMCC" in self.vehicles[vehicle.vin].supported_services:
            try:
                self.vehicles[vehicle.vin].guardian_mode = await self.hass.async_add_executor_job(
                    vehicle.get_guardian_mode_status
                )
            except HTTPError:
                # If not supported
                self.vehicles[vehicle.vin].guardian_mode = GuardianData(capable=False, active=False, expiry="0")
    ...
    async def async_call_service(self, service):
        ...
        if JLR_SERVICES[service.service]:
            ...
            if device_ids := service.data.get("entity_id") or area_ids:
                ...
                devices = dr.async_entries_for_config_entry(device_reg, self.config_entry.entry_id)
                vin_list = [
                    dict((x, y) for x, y in device.identifiers).get(DOMAIN)
                    for device in devices
                    if device.id in device_ids or device.id in entity_device_ids or device.area_id in area_ids
                ]
